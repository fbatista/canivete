...
class TournamentManagementTest < ApplicationSystemTestCase # rubocop:disable Metrics/ClassLength
  ...
  test "organizer can submit penalties and infractions" do
    ...
    visit organizer_event_participant_path(participant)
  ...
  test "organizer can view comprehensive tournament reports" do
    organizer = users(:organizer_user)
    tournament = events(:small_tournament_finished)
    sign_in_as(organizer)
  ...
  test "registration limits are enforced" do
    tournament = events(:small_tournament_registration_open)
    tournament.update!(max_participants: 2)

    # Fill tournament to capacity
    players = users.select(&:player?).first(2)
