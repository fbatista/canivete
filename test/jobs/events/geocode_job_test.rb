...
module Events
  class GeocodeJobTest < ActiveJob::TestCase
    test "successfully geocodes address and updates tournament location" do # rubocop:disable Metrics/BlockLength
      ...
      # Perform the job
      Events::GeocodeJob.perform_now(tournament)
    ...
    test "job is queued with correct arguments" do
      ...
      assert_enqueued_with(job: Events::GeocodeJob, args: [tournament]) do
        Events::GeocodeJob.perform_later(tournament)
      end
    end
    ...
    test "job is queued with correct queue" do
      ...
      assert_enqueued_with(job: Events::GeocodeJob, queue: "geocode") do
        Events::GeocodeJob.perform_later(tournament)
      end
    end
  end
