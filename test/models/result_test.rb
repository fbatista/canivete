...
class ResultTest < ActiveSupport::TestCase # rubocop:disable Metrics/ClassLength
  # ===== BASIC FIXTURE TEST =====
  ...
  test "belongs to event_participant and round" do
    result = results(:win_result)
    assert_not_nil result.event_participant
    assert_not_nil result.round
    assert_not_nil result.event # through round association
  end

  test "validates uniqueness of event_participant per round" do
    ...
    duplicate_result = Result.new(
      event_participant: existing_result.event_participant,
      round: existing_result.round,
      type: "Loss"
    )

    assert_not duplicate_result.valid?
    assert_includes duplicate_result.errors[:event_participant_id], "has already been taken"
  end
  ...
  test "publishable scope returns only results from finished rounds" do
    ...
    unfinished_result = Result.create!(
      event_participant: event_participants(:small_event_participant_one),
  ...
  test "create_or_update_by creates new result when not found" do
    ...
    assert_equal participant, new_result.event_participant
